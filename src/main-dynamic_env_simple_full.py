'''
In this script we test the dynamic_env_simple_v2 environment.
...
from utils.environments.dynamic_env_simple_full import Ego, NonEgo
...
road_down = non_ego_env.road_down
road_right = non_ego_env.road_right
non_ego_space = zono_op.union_hz_hz_v2(road_down, road_right)
non_ego = non_ego_env.car_outer_d
# Initialize obstacle based on region it is in
car_down = non_ego
car_right = zono_op.intersection_hz_hz(non_ego, road_right) # TODO: Just initialize this as a Hybrid zonotope of minimum order filled with zeros
#
...
for i in range(N):
    ...
    # name = f'brs_N_{i}'
    # ego_env.vis.fig.savefig(f'./results/dynamic_env_simple_full/brs/{name}.pdf', dpi=300)
    ...
    # name = f'obs_N_{i}'
    # plt.show()
    # ego_env.vis.fig.savefig(f'./results/dynamic_env_simple_full/obstacles/{name}.pdf', dpi=300)
    # plt.close(ego_env.vis.fig)
    # ego_env.vis.new_fig()
    #########################################################
    # ## OBS COMPLEMENT
    ...
    # ego_env.vis.fig.savefig(f'./results/dynamic_env_simple_full/obs_compl/{name}.pdf', dpi=300)
    # plt.close(ego_env.vis.fig)
    # ego_env.vis.new_fig()
    #########################################################
    ### BRS SAFE SPACE
    ...
    ego_env.vis.ax.set_title(f'iter = {i}, ng = {target.ng}, nc = {target.nc}, nb = {target.nb}', fontsize=16)
    name = f'brs_N_{i}'
    ego_env.vis.fig.savefig(f'./results/dynamic_env_simple_full/brs_safe_space/{name}.pdf', dpi=300)
    plt.close(ego_env.vis.fig)
    ego_env.vis.new_fig()
...
#     # TODO: Add the functionality of computing the non-safe space resulting from the non-ego vehicles.
#     # You need to create an environment just for the non-ego vehicle(s)

#     # TODO: Think about how you should correctly combine the environment of the ego-vehicle with the environment of the non-ego vehicle(s)
#     # That is because the computation of the BRS requires a different kind of environment, than the the environment used for the non-ego vehicle(s)
...
#     ego_env.vis.ax.set_title(f'iter = {i}, ng = {target.ng}, nc = {target.nc}, nb = {target.nb}', fontsize=16)
#     name = f'brs_N_{i}'
#     # ego_env.vis.fig.savefig(f'./results/dynamic_env_simple_full/{name}.pdf', dpi=300)
