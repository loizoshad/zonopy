...
print(f'Reduced {max_iter} time(s)')
print(f'Before reduction\t space: ng = {space.ng}\t nc = {space.nc}\t nb = {space.nb}')

for i in range(max_iter):
    space = zono_op.red_hz_scott(space)
    print(f'After reduction \t space: ng = {space.ng}\t nc = {space.nc}\t nb = {space.nb}')
...
for i in range(N):
    ...
    print(f'before: ng = {initial_space.ng}\t nc = {initial_space.nc}\t nb = {initial_space.nb}\t rank = {r}')
    ...
    env.vis.fig.savefig(f'./results/dynamic1/reduced/{name}.pdf', dpi=300)
