...
for i in range(N):
    ...
    print(f'Before reduction: ng = {target.ng}\t nc = {target.nc}\t nb = {target.nb}')
    target = zono_op.reduce_hz(target)
    print(f'After reduction: ng = {target.ng}\t nc = {target.nc}\t nb = {target.nb}')
    ...
    name = f'brs_N_{i}'
    plt.show()
    env.vis.fig.savefig(f'./results/reduction/reduced/{name}.pdf', dpi=300)
