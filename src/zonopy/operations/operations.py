...
import casadi as ca
import gurobipy as gp  # mixed-integer linear programming
import numpy as np

# from gurobipy import GRB
from gurobipy import *  # FIXME: should use the more explicit import above
...
from zonopy.tlt.nodes import AND, OR, UNTIL, set_node
from zonopy.tlt.tree import Tree
...
class ZonoOperations:
    ...
    def ms_z_z(self, z1: Zonotope, z2: Zonotope) -> Zonotope:
        ...
        assert (
            z1.dim == z2.dim
        ), f"Both zonotopes must have the same dimensionality, whereas z1.dimension = {z1.dimension} and z2.dimension = {z2.dimension}."
    ...
    def lt_z(self, M: np.ndarray, z: Zonotope) -> Zonotope:
        ...
        assert (
            M.shape[0] == M.shape[1] == z.dim
        ), f"Linear transformation matrix must be square \
    ...
    def ms_cz_cz(self, cz1: ConstrainedZonotope, cz2: ConstrainedZonotope) -> ConstrainedZonotope:
        ...
        assert (
            cz1.dim == cz2.dim
        ), f"Both constrained zonotopes must have the same dimensionality, \
        ...
        A = np.block(
            [[cz1.A, np.zeros((cz1.A.shape[0], cz2.A.shape[1]))], [np.zeros((cz2.A.shape[0], cz1.A.shape[1])), cz2.A]]
        )
    ...
    def intersection_cz_cz(self, cz1: ConstrainedZonotope, cz2: ConstrainedZonotope) -> ConstrainedZonotope:
        ...
        b = np.block([[cz1.b], [cz2.b], [cz2.C - cz1.C]])
    ...
    def intersection_hz_hz(self, hz1: HybridZonotope, hz2: HybridZonotope) -> HybridZonotope:
        ...
        Ac = np.vstack(
            (
                np.hstack((hz1.Ac, np.zeros((hz1.nc, hz2.ng)))),
        ...
        Ab = np.vstack(
            (
                np.hstack((hz1.Ab, np.zeros((hz1.nc, hz2.nb)))),
    ...
    def union_hz_hz(self, hz1: HybridZonotope, hz2: HybridZonotope) -> HybridZonotope:
        ...
        I = np.linalg.inv(np.block([[np.eye(hz1.dim), np.eye(hz1.dim)], [-np.eye(hz1.dim), np.eye(hz1.dim)]]))
        X1 = I @ np.block([[hz2.Gb @ np.ones((hz2.nb, 1)) + hz1.C], [hz1.Gb @ np.ones((hz1.nb, 1)) + hz2.C]])
        Gb_hat = X1[0 : hz1.dim, :]
        C_hat = X1[hz1.dim :, :]

        I = np.linalg.inv(np.block([[-np.eye(hz1.nc), np.eye(hz1.nc)], [np.eye(hz1.nc), np.eye(hz1.nc)]]))
        X2 = I @ np.block([[hz1.b], [-hz1.Ab @ np.ones((hz1.nb, 1))]])
        Ab1_hat = X2[0 : hz1.nc, :]
        b1_hat = X2[hz1.nc :, :]

        I = np.linalg.inv(np.block([[-np.eye(hz2.nc), np.eye(hz2.nc)], [np.eye(hz2.nc), np.eye(hz2.nc)]]))
        X3 = I @ np.block([[-hz2.Ab @ np.ones((hz2.nb, 1))], [hz2.b]])
        Ab2_hat = X3[0 : hz2.nc, :]
        b2_hat = X3[hz2.nc :, :]
        ...
        Gb = np.block([[hz1.Gb, hz2.Gb, Gb_hat]])
        ...
        Ab = np.block(
            [[hz1.Ab, np.zeros((hz1.nc, hz2.nb)), Ab1_hat], [np.zeros((hz2.nc, hz1.nb)), hz2.Ab, Ab2_hat], [Ab3]]
        )

        b = np.block([[b1_hat], [b2_hat], [b3]])
    ...
    def one_step_brs_hz(self, X: HybridZonotope, T: HybridZonotope, D: np.ndarray) -> HybridZonotope:
        ...
        Gc = np.block([[X.Gc[: T.dim, :], np.zeros((T.dim, T.ng))]])

        Gb = np.block([[X.Gb[: T.dim, :], np.zeros((T.dim, T.nb))]])

        C = X.C[: T.dim, :]

        Ac = np.block([[X.Ac, np.zeros((X.nc, T.ng))], [np.zeros((T.nc, X.ng)), T.Ac], [D @ X.Gc, -T.Gc]])

        Ab = np.block([[X.Ab, np.zeros((X.nc, T.nb))], [np.zeros((T.nc, X.nb)), T.Ab], [D @ X.Gb, -T.Gb]])

        b = np.block([[X.b], [T.b], [T.C - D @ X.C]])

        return HybridZonotope(Gc, Gb, C, Ac, Ab, b)

    def brs_hz(
        self, X: HybridZonotope, T: HybridZonotope, D: np.ndarray, N: int, visualize=False, env=None
    ) -> HybridZonotope:
        """
        ...
        for _i in range(N):
            ...
            if self.visualizer is not None and visualize:
                start_time = time.perf_counter()
                self.visualizer.vis_hz_brs(T, title="BRS", legend_labels=["$\\mathscr{BRS}$"], add_legend=True, env=env)
                end_time = time.perf_counter()
