...
class HybridZonotope:
    def __init__(
        self, Gc: np.ndarray, Gb: np.ndarray, C: np.ndarray, Ac: np.ndarray, Ab: np.ndarray, b: np.ndarray
    ) -> None:
        """
        ...
        assert (
            Ac.shape[0] == Ab.shape[0] == b.shape[0]
        ), "Number of constraints must be consinstent across Ac, Ab, and b"
