...
from zonopy.sets.constrained_zonotopes import ConstrainedZonotope
...
# TLT
from zonopy.tlt.nodes import AND, OR, UNTIL, WUNTIL, set_node
...
class ZonoVisualizer:
    ...
    def init_brs_plot(self, env=None):
        #####################################################################
        #                           Initialization                          #
        #####################################################################
        if env is not None:
            ...
            y_min = env.y_min
            y_max = env.y_max
            self.samples_x = env.samples_x
            self.samples_y = env.samples_y
            self.already_contained_points = env.initial_points
            self.x_step = env.x_step
            self.y_step = env.y_step
            self.max_dist_x = env.max_dist_x
            self.max_dist_y = env.max_dist_y
            self.max_dist_diag = env.max_dist_diag
            ...
            self.already_contained_points = np.array([[8.0, 9.0], [8.0, 6.5], [8.0, -9.0], [8.0, -6.5]])
            # Update the flag for already contained points
            for p in self.already_contained_points:
                x_idx = np.argmin(np.abs(self.x_space - p[0]))
                y_idx = np.argmin(np.abs(self.y_space - p[1]))
    ...
    def vis_z(self, zonotopes: list, title="", xlabel=r"$x_1$", ylabel=r"$x_2$", legend_labels=[], add_legend=True):
        ...
        assert all(z.dim == 2 for z in zonotopes), "Zonotope(s) must be 2D (i.e., z.dim = 2)"
        ...
        for z in zonotopes:
            ...
            self.min_x = min(np.min(vertices[0, :]), self.min_x)
            self.max_x = max(np.max(vertices[0, :]), self.max_x)
            self.min_y = min(np.min(vertices[1, :]), self.min_y)
            self.max_y = max(np.max(vertices[1, :]), self.max_y)

        x_margin = 0.2 * (self.max_x - self.min_x)
        y_margin = 0.2 * (self.max_y - self.min_y)
        self.ax.set_xlim(self.min_x - x_margin, self.max_x + x_margin)
        self.ax.set_ylim(self.min_y - y_margin, self.max_y + y_margin)
        ...
        if add_legend:
            self.ax.legend(handles, labels, loc="upper left")  # Add the legend
    ...
    def vis_cz(
        ...
        """
        # Assert that the zonotopes are 2D
        assert all(cz.dim == 2 for cz in czonotopes), "Constrained Zonotope(s) must be 2D (i.e., cz.dim = 2)"
    ...
    def vis_hz(
        ...
        """
        assert all(hz.dim == 2 for hz in hzonotopes), "Hybrid Zonotope(s) must be 2D (i.e., hz.dim = 2)"
    ...
    def vis_hz_brs(
        ...
        """
        assert hz.dim == 2, "Hybrid Zonotope must be 2D (i.e., hz.dim = 2)"

        self.zono_op.warm_start = None

        if env is not None:
            self.init_brs_plot(env=env)
            ...
            for y_i, y in enumerate(self.y_space):
                ...
                if 1 in self.is_already_contained[y_i, max(0, x_i - self.max_dist_y) : x_i]:
                    ...
                    for q in range(self.max_dist_diag):
                        ...
                        if self.is_already_contained[
                            min(self.samples_y - 1, y_i + q), min(self.samples_x - 1, x_i + q)
                        ]:
                            close_enough = True
                            break
                ...
                if close_enough:
                    ...
                    if self.zono_op.is_inside_hz(hz, p):
                        ...
                        self.already_contained_points = np.append(
                            self.already_contained_points, p.reshape(1, -1), axis=0
                        )

                        # Add to the plot a circle of radius 'step'
                        # plt.scatter(p[0], p[1], marker = '.', s = 350, color = '#4783FC', alpha = 1.0)
                        self.ax.scatter(p[0], p[1], marker=".", s=350, color="#4783FC", alpha=1.0)

                        # Just to fill in the gaps between the circles
                        # plt.scatter(p[0], p[1] + self.y_step/2.0, marker = '.', s = 250, color = '#4783FC', alpha = 1.0)
                        # plt.scatter(p[0] - self.x_step/2.0, p[1], marker = '.', s = 250, color = '#4783FC', alpha = 1.0)
    ...
    def vis_patches(self):
        ...
        vertices = np.array([[-2.1, 1.0], [1.5, 1.0], [1.5, -1.0], [-2.1, -1.0]])

        # Create a line between the first two vertices
        road_line = Polygon(vertices, closed=True, fill=False, color="white", linestyle="--", linewidth=6)
        self.ax.add_patch(road_line)

        # Parking spot 1 line
        vertices = np.array([[1.910, 0.6], [1.910, 0.6], [1.910, 0.2], [1.910, 0.2]])
        parking_spot_1_line = Polygon(vertices, closed=True, fill=False, color="white", linestyle="--", linewidth=2)
        self.ax.add_patch(parking_spot_1_line)
        # Parking spot 2 line
        vertices = np.array([[1.910, -0.2], [1.910, -0.2], [1.910, -0.6], [1.910, -0.6]])
        parking_spot_2_line = Polygon(vertices, closed=True, fill=False, color="white", linestyle="--", linewidth=2)
        self.ax.add_patch(parking_spot_2_line)

        # Parking spot 3 line
        vertices = np.array([[-0.9, 0.6], [-0.5, 0.6], [-0.5, 0.6], [-0.9, 0.6]])
        parking_spot_3_line = Polygon(vertices, closed=True, fill=False, color="white", linestyle="--", linewidth=2)
        self.ax.add_patch(parking_spot_3_line)

        # Parking spot 4 line
        vertices = np.array([[-0.105, 0.6], [0.305, 0.6], [-0.105, 0.6], [0.305, 0.6]])
        parking_spot_4_line = Polygon(vertices, closed=True, fill=False, color="white", linestyle="--", linewidth=2)
        self.ax.add_patch(parking_spot_4_line)
    ...
    def vis_images(self):
        ...
        """
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[1.9, 2.5, 1.4, 0.6]  # left  # right  # top  # bottom
        )
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[1.9, 2.5, 0.2, -0.2]  # left  # right  # top  # bottom
        )
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[1.9, 2.5, -0.6, -1.4]  # left  # right  # top  # bottom
        )

        # INTERNAL OBSTACLES
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[-1.7, 1.1, 0.0, -0.6]  # left  # right  # top  # bottom
        )
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[-1.7, -0.9, 0.6, 0.0]  # left  # right  # top  # bottom
        )
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[0.3, 1.1, 0.6, 0.0]  # left  # right  # top  # bottom
        )
        self.ax.imshow(
            self.images, alpha=1.0, zorder=100, extent=[-0.5, -0.1, 0.6, 0.0]  # left  # right  # top  # bottom
        )
    ...
    def vis_cz2(
        ...
        for cz in czonotopes:
            ...
            poly = Polygon(
                ...
                edgecolor=(color[0], color[1], color[2]),
                alpha=0.6,
            )
            self.ax.add_patch(poly)
...
class TreeVisualizer:
    ...
    def vis_tree(self, tree):
        ...
        # Save and open graph
        self.dot.render("./results/tlt/test.gv")
        self.dot.view()
